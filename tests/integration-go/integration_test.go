...
func TestWasmCrypto(t *testing.T) {
	...
	xprivChildB58 := []byte("xprv9uGHcGUn4FQYZ6LYvVdgdAA8kp3sEG3ptWGzubhwnZaj8CefeqQJ4AWXedszNL3woGbQadP5og3GfZfBcNt5iNKRk4gXcHVcuRzTRbVKj9J")
	childIndex := int32(1)
	check(err)
	...
	wasmXPrivPub, err7 := signer.PublicKeyXPriv(xprivChildB58, false)
	wasmXPrivPubCompressed, err8 := signer.PublicKeyXPriv(xprivChildB58, true)
	wasmXPrivChildPub, err9 := signer.PublicKeyXPrivChild(xprivB58, false, childIndex)
	wasmXPrivChildPubCompressed, err10 := signer.PublicKeyXPrivChild(xprivB58, true, childIndex)
	...
	expectedPubKeyChild := []byte{
		4, 168, 233, 30, 80, 78, 205, 19, 187, 212, 38, 50, 139, 36, 193, 212, 82, 229, 29, 33, 233, 149, 13, 233, 79, 129, 1, 235, 130, 102, 169, 129, 217, 112, 72, 22, 174, 144, 227, 100, 230, 39, 98, 177, 251, 254, 211, 170, 184, 80, 96, 136, 183, 60, 125, 95, 172, 135, 3, 13, 178, 199, 183, 197, 137,
	}
	expectedPubKeyChildCompressed := []byte{
		3, 168, 233, 30, 80, 78, 205, 19, 187, 212, 38, 50, 139, 36, 193, 212, 82, 229, 29, 33, 233, 149, 13, 233, 79, 129, 1, 235, 130, 102, 169, 129, 217,
	}
	...
	assert.Equal(t, expectedPubKeyChild, wasmXPrivPub)
	assert.Equal(t, expectedPubKeyChildCompressed, wasmXPrivPubCompressed)
	assert.Equal(t, expectedPubKeyChild, wasmXPrivChildPub)
	assert.Equal(t, expectedPubKeyChildCompressed, wasmXPrivChildPubCompressed)
